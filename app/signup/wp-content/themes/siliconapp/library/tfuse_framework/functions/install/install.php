...
<?php
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
            ...
            if ( $this->r ) {
                ...
                $cat['category_parent'] = ( !empty($cat['category_parent']) ) ? "$slug_prefix-".$cat['category_parent'] : '';
	...
	function process_posts() {
		foreach ( $this->posts as $post ) {
            ...
            //if( 'post' == $post['post_type'] || 'page' == $post['post_type'] ) $post['guid'] = '';
	...
	function process_attachment( $post, $url ) {
		...
		$post_id = wp_insert_attachment( $post, $giud );

		wp_update_attachment_metadata( $post_id, wp_generate_attachment_metadata( $post_id, $giud ) );
		...
		foreach($import_optionsArr as $option) {
			...
			if($option['name'] == 'upload_link') {
				...
				if($option['option'] == 'pag') {
                    ...
                    //TODO, trebuie de verificat si de vazut de ce e +1
    ...
    function greet() {
		...
		$output .= '<div style="clear:both;height:10px;"></div>
					<div class="demoinstall">
						<div class="tfuse_install_submit">
							<form method="post" action="'.get_bloginfo('url').'/wp-admin/admin.php?page=tfuse&step=2&import_id='.$this->id.'">
								<p class="submit"><input type="submit" class="button" id="install_btn" value="Install Demo Version &raquo;" /></p>
							</form>
						</div>
						<img class="tfuse_install_icon" src="'. ADMIN_IMAGES .'/auto_install_icon.png" width="33" height="33" />
						<h3>Auto Install</h3>
						<p>
					...
					<div class="skipinstall">
						<div class="tfuse_install_submit">
							<form method="post" action="'.get_bloginfo('url').'/wp-admin/admin.php?page=tfuse&step=4">
								<p class="submit"><input name="skip_installation" type="submit" value="Skip Installation &raquo;" /></p>
							</form>
						</div>
						<img class="tfuse_install_icon" src="'. ADMIN_IMAGES .'/skip_install_icon.png" width="33" height="33" />
		...
		$output .= '<div class="install_loading">'."\n";
		$output .= '
					<center>
					<br />
					<img src="'.get_bloginfo('template_directory').'/library/tfuse_framework/images/loading.gif" /> <br />
					<p>Installing ... </p>
					<br />
					</center>
    ...
    function TFUSE_Import() {
        ...
        $prefix = $tfuse->prefix;
        $this->slug_prefix = sanitize_title(get_option("{$prefix}_themename"));
